...
class DiscordApp {
  ...
  toggleEmojiPicker() {
    ...
    this.currentEmojiPicker.style.bottom = '60px';
    ...
    this.currentEmojiPicker.addEventListener('emojiSelect', (e) => {
      const messageInput = document.getElementById('message-input');
      if (messageInput) {
        messageInput.value += e.detail.emoji;
        messageInput.focus();
      }
      this.closeEmojiPicker();
    });
  ...
  showEmojiSuggestions(suggestions, input, startPos) {
    ...
    suggestionBox.className = 'absolute bottom-16 left-4 bg-[#36393f] border border-[#40444b] rounded-lg shadow-xl p-2 max-h-40 overflow-y-auto z-50';
