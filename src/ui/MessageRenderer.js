...
export class MessageRenderer {
  ...
  processMessageContent(content) {
    ...
    // Process URLs
    content = content.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" class="text-[#00aff4] hover:underline">$1</a>');
    
    // Process code blocks
    content = content.replace(/```([\s\S]*?)```/g, '<pre class="bg-[#2f3136] p-2 rounded mt-1 overflow-x-auto"><code>$1</code></pre>');
  ...
  addMessageActions(container, message) {
    ...
    actions.className = 'message-actions absolute top-0 right-4 bg-[#36393f] border border-[#40444b] rounded shadow-lg px-1 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1';
    actions.innerHTML = `
